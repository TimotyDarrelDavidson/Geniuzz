...
<script>
    function confirmDelete(eid) {
        const userConfirmed = confirm("Are you sure you want to remove this quiz? This action cannot be undone.");
        if (userConfirmed) {
            window.location.href = `update.php?q=rmquiz&eid='$eid'`;
        }
    }
    function confirmDeleteUser(email) {
        const userConfirmed = confirm("Are you sure you want to remove this user? This action cannot be undone.");
        if (userConfirmed) {
            window.location.href = "update.php?demail=" + encodeURIComponent(email);;
    ...
    <nav class="navbar navbar-default title1">
        <div class="container-fluid">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="Javascript:void(0)"><b>Geniuzz | Welcome, Admin</b></a>
            </div>
                ...
                <?php 
                    if(@$_GET['q']==1) 
                    {
                        ...
                        while($row = mysqli_fetch_array($result)) 
                        {
                            ...
                            <td>
                                <center>
                                    <a title="Delete User" href="update.php?demail='.$email.'" onclick="confirmDeleteUser(\''.$email.'\')">
                ...
                <?php 
                    if(@$_GET['q']==5) 
                    {
                        ...
                        while($row = mysqli_fetch_array($result)) {
                            ...
                            <td><center><b><a href="update.php?q=rmquiz&eid='.$eid.'" onclick="confirmDelete(\''.$eid.'\')"  class="pull-right btn sub1" style="margin:0px;background:red;color:black"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Remove</b></span></a></b></center></td></tr>';
                        }
                        $c=0;
                        echo '</table></div></div>';
                    }
