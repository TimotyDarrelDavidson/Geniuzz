<?php
  ...
  if(isset($_SESSION['key']))
  {
    ...
    {
      $eid=@$_GET['eid'];
      echo "EID received: " . htmlspecialchars($eid);
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addqns' && $_SESSION['key']=='admin') 
    {
      ...
      for($i=1;$i<=$n;$i++)
      {
        $qid = uniqid('14', true); // Tambahkan entropi dengan `true` dan prefix
        $qns=$_POST['qns'.$i];
  ...
  //pengelolaan jawaban & skor
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    $eid=@$_GET['eid'];//quiz id
    $sn=@$_GET['n'];
    $total=@$_GET['t'];
    $ans=$_POST['ans'];
    $qid=@$_GET['qid'];//question id
    $q=mysqli_query($con,"SELECT * FROM answer WHERE qid='$qid' " ); //question
    $totalquestion = 0;
    ...
    if($ans == $ansid)
    {
      ...
      $r++;
      $s=$s+$right*100/($totalquestion*$right*$total);
    ...
    else
    {
      ...
      $w++;
      $s=$s-$wrong*100/($totalquestion*$wrong*$total);
      if($s<0){
        $s=0;
      }
