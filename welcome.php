...
<head>
    ...
    <script>
        function confirmStart(eid,total) {
        const userConfirmed = confirm("Are you ready to start?");
        if (userConfirmed) {
            window.location.href = `welcome.php?q=quiz&step=2&eid='.$eid.'&n=1&t='.$total.''`;
           
        }
    }
    function confirmRestart(eid,total) {
        const userConfirmed = confirm("Are you ready to restart this quiz? This action will reset your existing score");
        if (userConfirmed) {
            window.location.href = `update.php?q=quizre&step=25&eid='.$eid.'&n=1&t='.$total.'`;
        }
    }
    </script>
</head>
<body style="background-image: url(image/stock-background.jpg)">
        ...
        <div class="row">
            <div class="col-md-12">
                <?php if(@$_GET['q']==1) 
                {
                    ...
                    if($rowcount == 0){
                        echo '<tr><td><center>'.$c++.'</center></td><td><center>'.$title.'</center></td><td><center>'.$total.'</center></td><td><center>'.$right*$total.'</center></td><td><center><b><a href="welcome.php?q=quiz&step=2&eid='.$eid.'&n=1&t='.$total.'" onclick="confirmStart(\''.$eid.'\',\''.$total.'\')" class="btn sub1" style="color:black;margin:0px;background:#1de9b6"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Start</b></span></a></b></center></td></tr>';
                    }
                    else
                    {
                    echo '<tr style="color:#99cc32"><td><center>'.$c++.'</center></td><td><center>'.$title.'&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></center></td><td><center>'.$total.'</center></td><td><center>'.$right*$total.'</center></td><td><center><b><a href="update.php?q=quizre&step=25&eid='.$eid.'&n=1&t='.$total.'" onclick="confirmRestart(\''.$eid.'\',\''.$total.'\')" class="pull-right btn sub1" style="color:black;margin:0px;background:red"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Restart</b></span></a></b></center></td></tr>';
                    }
                    }
                    $c=0;
                    echo '</table></div></div>';
